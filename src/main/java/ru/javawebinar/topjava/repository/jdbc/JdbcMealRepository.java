...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        } else if(namedParameterJdbcTemplate.update("UPDATE meals SET userId=:userId, " +
                    "dateTime=:dateTime, description=:description, calories=:calories WHERE id=:id AND userId=:userId", map) == 0) {
            return null;
        }
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE userId=? AND dateTime BETWEEN ? AND ? ORDER BY dateTime DESC", ROW_MAPPER,
